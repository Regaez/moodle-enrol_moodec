...
/**
 * Enrol moodec external PHPunit tests
 *
...
class enrol_moodec_externallib_testcase extends externallib_advanced_testcase {
	...
	public function test_enrol_users() {
		...
		enrol_moodec_external::enrol_users(array(
		...
		$this->unassignUserCapability('enrol/moodec:enrol', $context1->id, $roleid);
		try {
			enrol_moodec_external::enrol_users(array(
				array('roleid' => 3, 'userid' => $user1->id, 'courseid' => $course1->id),
			));
		...
		$this->assignUserCapability('enrol/moodec:enrol', $context1->id, $roleid);
		...
		// Call for course without moodec instance.
		$DB->delete_records('user_enrolments');
		...
		try {
			...
			$this->fail('Exception expected if course does not have moodec instance');
		} catch (moodle_exception $e) {
