...
/**
 * Auxiliary moodec user enrolment lib, the main purpose is to lower memory requirements...
 *
 ...
 * Enrol candidates.
 */
class enrol_moodec_potential_participant extends user_selector_base {
	protected $enrolid;
	...
	protected function get_options() {
		...
		$options['file'] = 'enrol/moodec/locallib.php';
		return $options;
	}
}
 ...
 * Enrolled users.
 */
class enrol_moodec_current_participant extends user_selector_base {
	protected $courseid;
	protected $enrolid;
...
/**
 * A bulk operation for the moodec enrolment plugin to edit selected users.
 *
 * @copyright 2011 Sam Hemelryk
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class enrol_moodec_editselectedusers_operation extends enrol_bulk_enrolment_operation {
	...
	public function get_title() {
		return get_string('editselectedusers', 'enrol_moodec');
	}
	...
	public function process(course_enrolment_manager $manager, array $users, stdClass $properties) {
		...
		if (!has_capability("enrol/moodec:manage", $manager->get_context())) {
			return false;
		}
	 ...
	 * @return enrol_moodec_editselectedusers_form
	 */
	public function get_form($defaultaction = null, $defaultcustomdata = null) {
		global $CFG;
		require_once $CFG->dirroot . '/enrol/moodec/bulkchangeforms.php';
		return new enrol_moodec_editselectedusers_form($defaultaction, $defaultcustomdata);
	}
}

/**
 * A bulk operation for the moodec enrolment plugin to delete selected users enrolments.
 *
 * @copyright 2011 Sam Hemelryk
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class enrol_moodec_deleteselectedusers_operation extends enrol_bulk_enrolment_operation {
	 ...
	 */
	public function get_title() {
		return get_string('deleteselectedusers', 'enrol_moodec');
	}
	...
	public function get_form($defaultaction = null, $defaultcustomdata = null) {
		...
		$defaultcustomdata['message'] = get_string('confirmbulkdeleteenrolment', 'enrol_moodec');
		$defaultcustomdata['button'] = get_string('unenrolusers', 'enrol_moodec');
		return new enrol_moodec_deleteselectedusers_form($defaultaction, $defaultcustomdata);
	}
	...
	public function process(course_enrolment_manager $manager, array $users, stdClass $properties) {
		...
		if (!has_capability("enrol/moodec:unenrol", $manager->get_context())) {
			return false;
		}
		foreach ($users as $user) {
			foreach ($user->enrolments as $enrolment) {
...
/**
 * Migrates all enrolments of the given plugin to enrol_moodec plugin,
 ...
 */
function enrol_moodec_migrate_plugin_enrolments($enrol) {
	global $DB;
	...
	$sql = "SELECT e.id, e.courseid, e.status, MIN(me.id) AS mid, COUNT(ue.id) AS cu
         ...
         LEFT JOIN {enrol} me ON (me.courseid = e.courseid AND me.enrol='moodec')
             WHERE e.enrol = :enrol
	...
	foreach ($rs as $e) {
		$minstance = false;
		if (!$e->mid) {
			// moodec instance does not exist yet, add a new one.
			$course = $DB->get_record('course', array('id' => $e->courseid), '*', MUST_EXIST);
			if ($minstance = $DB->get_record('enrol', array('courseid' => $course->id, 'enrol' => 'moodec'))) {
				// Already created by previous iteration.
				$e->mid = $minstance->id;
			} else if ($e->mid = $moodecplugin->add_default_instance($course)) {
		...
		if (!$minstance) {
			...
			debugging('Failed to find moodec enrolment instance', DEBUG_DEVELOPER);
			continue;
		}
		...
		// Migrate to moodec enrol instance.
		$params = array('id' => $e->id, 'mid' => $e->mid);
 ...
 */
function enrol_moodec_get_potential_cohorts($context, $enrolid, $search = '', $page = 0, $perpage = 25, $addedenrollment = 0) {
	global $CFG;
